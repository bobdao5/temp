...
import { isAccessControlError, isStreamOfflineError, getMediaSourceType } from '@livepeer/core';
...
const useSourceMimeTyped = ({
  ...
  const [type, sourceMimeTyped] = React.useMemo(() => {
    ...
    const sources = playbackUrls.length > 0 ? playbackUrls.map(p => p.url) : typeof src === 'string' ? [src] : src;
    if (!sources) {
      return defaultValue;
    }
    ...
    const mediaSourceTypes = authenticatedSources.map(s => typeof s === 'string' ? getMediaSourceType(s) : null).filter(s => s);

    // we filter by either audio or video/hls
    return mediaSourceTypes?.[0]?.type === 'audio' ? ['audio', mediaSourceTypes.filter(s => s.type === 'audio')] : mediaSourceTypes?.[0]?.type === 'video' || mediaSourceTypes?.[0]?.type === 'hls' || mediaSourceTypes?.[0]?.type === 'webrtc' ? ['video', mediaSourceTypes.filter(s => s.type === 'video' || s.type === 'hls' || s.type === 'webrtc')] : defaultValue;
...
const usePlayer = ({
  ...
  const playerProps = React.useMemo(() => ({
    ...
    onPlaybackStatusUpdate,
    playbackStatusSelector
  }), [playerRef, playbackId, autoPlay, muted, poster, loop, objectFit, controls, priority, playbackError, onPlaybackError, _isCurrentlyShown, viewerId, onPlaybackStatusUpdate, playbackStatusSelector]);
  const mediaControllerProps = React.useMemo(() => ({
    autoPlay,
  ...
  const controlsContainerProps = React.useMemo(() => ({
    ...
    error: playbackError
  }), [hidePosterOnPlayed, showLoadingSpinner, loadingText, showUploadingIndicator, playbackError]);
  const props = React.useMemo(() => ({
    autoPlay,
    children,
    controls,
    ...
    objectFit,
    renderChildrenOutsideContainer
  }), [autoPlay, children, controls, playbackId, src, theme, title, poster, loop, jwt, refetchPlaybackInfoInterval, autoUrlUpload, showTitle, aspectRatio, objectFit, renderChildrenOutsideContainer]);
...
export { useBaseSlider, useBroadcast, useControlsContainer, useFullscreenButton, usePlayButton, usePlayer, useProgress, useTimeDisplay, useVolume };
