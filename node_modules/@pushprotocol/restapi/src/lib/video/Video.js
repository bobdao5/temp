...
class Video {
    constructor({ signer, chainId, pgpPrivateKey, env = constants_1.default.ENV.PROD, setData, callType = constants_2.VIDEO_CALL_TYPE.PUSH_VIDEO, onReceiveStream = () => {
        return;
    }, }) {
                        ...
                        if ((0, isJSON_1.default)(data)) {
                            ...
                            if (parsedData.type === 'endCall') {
                                ...
                                if (this.callType === constants_2.VIDEO_CALL_TYPE.PUSH_VIDEO ||
                                    (this.callType === constants_2.VIDEO_CALL_TYPE.PUSH_SPACE &&
                                        ((_e = parsedData === null || parsedData === void 0 ? void 0 : parsedData.details) === null || _e === void 0 ? void 0 : _e.type) === constants_2.SPACE_DISCONNECT_TYPE.STOP)) {
    ...
    connect(options) {
        ...
        try {
            ...
            // setup error handler
            this.peerInstances[peerAddress].on('error', (err) => {
                console.log('error in connect', err);
                const incomingIndex = (0, getIncomingIndexFromAddress_1.default)(this.data.incoming, peerAddress);
            ...
            (_a = this.peerInstances[peerAddress]) === null || _a === void 0 ? void 0 : _a.signal(signalData);
            // send the addresses the local peer is connected to remote peer
            const connectedAddresses = (0, getConnectedAddresses_1.default)({
                incomingPeers: this.data.incoming,
            });
            this.peerInstances[peerAddress].send(JSON.stringify({
                type: 'connectedAddresses',
            ...
            this.setData((oldData) => {
                return (0, immer_1.produce)(oldData, (draft) => {
                    const incomingIndex = (0, getIncomingIndexFromAddress_1.default)(oldData.incoming, peerAddress);
                    draft.incoming[incomingIndex].status = types_1.VideoCallStatus.CONNECTED;
                });
            });
        }
    ...
    disconnect(options) {
        ...
        try {
            const incomingIndex = (0, getIncomingIndexFromAddress_1.default)(this.data.incoming, peerAddress);
            console.log('disconnect', 'status', this.data.incoming[incomingIndex].status);
            if (this.data.incoming[incomingIndex].status === types_1.VideoCallStatus.CONNECTED) {
                (_a = this.peerInstances[peerAddress]) === null || _a === void 0 ? void 0 : _a.send(JSON.stringify({ type: 'endCall', value: true, details }));
                (_b = this.peerInstances[peerAddress]) === null || _b === void 0 ? void 0 : _b.destroy();
            }
            else {
            ...
            (_c = this.peerInstances[peerAddress]) === null || _c === void 0 ? void 0 : _c.destroy();
    ...
    enableVideo(options) {
        ...
        if (this.data.local.video !== state) {
            ...
            const incomingIndex = (0, getIncomingIndexFromAddress_1.default)(this.data.incoming, peerAddress);
            if (this.data.incoming[incomingIndex].status === types_1.VideoCallStatus.CONNECTED) {
                (_a = this.peerInstances[peerAddress]) === null || _a === void 0 ? void 0 : _a.send(JSON.stringify({
                    type: 'isVideoOn',
                    value: state,
                }));
            }
    ...
    enableAudio(options) {
        ...
        if (this.data.local.audio !== state) {
            ...
            const incomingIndex = (0, getIncomingIndexFromAddress_1.default)(this.data.incoming, peerAddress);
            if (this.data.incoming[incomingIndex].status === types_1.VideoCallStatus.CONNECTED) {
                (_a = this.peerInstances[peerAddress]) === null || _a === void 0 ? void 0 : _a.send(JSON.stringify({ type: 'isAudioOn', value: state }));
            }
            if (this.data.local.stream) {
                if (state) {
